...
def main(params):
    ...
    imgs_val = filter_question(imgs_val, atoi)
    ...
    # tokenization and preprocessing validation question
    ...
    ques_val, ques_length_val, question_id_val = encode_question(imgs_val, params, wtoi)
    ...
    unique_img_val, img_pos_val = get_unqiue_img(imgs_val)
    unique_img_test, img_pos_test = get_unqiue_img(imgs_test)
    print(max(img_pos_train))
    print(max(img_pos_val))
    exit()
    '''
    img_pos_train = get_img(imgs_train)
    img_pos_val = get_img(imgs_val)
    img_pos_test = get_img(imgs_test)
    # get the answer encoding.
    A = encode_answer(imgs_train, atoi)
    B = encode_answer(imgs_val, atoi)
    ...
    f.create_dataset("answers_val", dtype='uint32', data=B)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_val_json', required=True, help='input json file to process into hdf5')
